...
async def create_medication_request(
    ...
    await service.create_medication_request(
        session=session, medication_request=medication_request
    )
...
async def get_filtered_medication_request(
    ...
    """
    service = MedicationRequestService()
    response = await service.get_medication_request(
        session=session, status=status, start_date=start_date, end_date=end_date
    )
    return response
...
async def update_medication_request(
    ...
    service = MedicationRequestService()
    await service.patch_medication_request(
        session=session,
