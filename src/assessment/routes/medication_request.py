...
@router.get("/medications-request", response_model_exclude_none=True)
async def get_filtered_medication(
    ...
    """
    service = MedicationRequestService()
    await service.get_medication_request(session=session, status=status, start_date=start_date, end_date=end_date)